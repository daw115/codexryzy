...
def document_exists(source_type: str, external_id: str) -> bool:
    """Return True if the document is already in the knowledge base (skip download + analysis)."""
    import urllib.parse
    headers = {}
    if WORK_ASSISTANT_API_KEY:
        headers["X-API-Key"] = WORK_ASSISTANT_API_KEY
    params = urllib.parse.urlencode({"source_type": source_type, "external_id": external_id})
    try:
        result = http_json("GET", f"{WORK_ASSISTANT_API_URL}/v1/documents/exists?{params}", headers=headers)
        return bool(result.get("exists"))
    except Exception:
        return False  # on any error, proceed with ingestion
...
def queue_enrichment_if_needed(
    *,
    ...
    if not AUTO_QUEUE_ENRICHMENT:
        return None

    topics = [topic for topic in parsed_analysis.get("topics") or [] if isinstance(topic, str) and topic.strip()]
    if not topics:
        return None

    selected_topics = topics[:ENRICH_MAX_TOPICS]
    body = {
        "topics": [{"name": topic, "confidence": None} for topic in selected_topics],
        "queries": [f"latest updates {topic}" for topic in selected_topics],
        "source_document_ids": [source_document_id],
        "source_revision_ids": [source_revision_id],
        "freshness_days": ENRICH_FRESHNESS_DAYS,
        "max_results": ENRICH_MAX_RESULTS,
        "allow_domains": ENRICH_ALLOW_DOMAINS,
        "notes": notes,
    }
    headers = {}
    if WORK_ASSISTANT_API_KEY:
    ...
    return http_json(
        "POST",
        f"{WORK_ASSISTANT_API_URL}/v1/enrichment/web",
        body=body,
        headers=headers,
    )
