...
async function callClaude(apiKey: string, contentParts: any[], tools: any[], toolName: string, system?: string) {
...
6. For each pattern, provide keywords and a suggested category`;
    ...
    // System message passed as top-level system parameter for Claude API
    const response = await callClaude(
      ANTHROPIC_API_KEY,
      contentParts,
      tools,
      "save_knowledge",
      "You are a knowledge extraction engine. Respond only via tool calls."
    );
    ...
    const knowledge = extractToolInput(aiResult);
    if (!knowledge) throw new Error("No tool call in AI response");
