...
serve(async (req) => {
  ...
  try {
    ...
    async function callAI(contentParts: any[], tools: any[], toolChoice: any) {
      ...
      const response = await callClaude(ANTHROPIC_API_KEY!, contentParts, tools, toolName);
    ...
    if (selectedMode === "crop-split") {
      ...
      // All frames are also caption sources (Claude will read captions from full frames)
      const allFrameRefs = [
