...
export default function PdfSlidesUploader({ meetingId, recordingFilename, onComplete }: Props) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      console.log(`[PdfUploader] Starting render of ${numPages} pages, stem=${stem}, userId=${user.id}`);
      for (let i = 1; i <= numPages; i++) {
        setProgress(`Strona ${i}/${numPages}...`);
        try {
          const page = await pdf.getPage(i);
          const viewport = page.getViewport({ scale: 2.0 }); // high res
          console.log(`[PdfUploader] Page ${i}: viewport ${viewport.width}x${viewport.height}`);
          ...
          console.log(`[PdfUploader] Page ${i}: rendered`);
          ...
          console.log(`[PdfUploader] Page ${i}: blob ${blob.size} bytes`);

          const path = `${user.id}/slides/${stem}/page_${String(i).padStart(3, "0")}.jpg`;
          console.log(`[PdfUploader] Page ${i}: uploading to ${path}`);
          const { error: uploadErr } = await supabase.storage
            .from("recordings")
            .upload(path, blob, { contentType: "image/jpeg", upsert: true });

          if (uploadErr) {
            console.warn(`[PdfUploader] Upload page ${i} error:`, uploadErr.message);
            continue;
          }

          console.log(`[PdfUploader] Page ${i}: uploaded OK`);
          slidePaths.push({ path, page: i, ts_formatted: `P${i}` });
        } catch (pageErr: any) {
          console.error(`[PdfUploader] Page ${i} failed:`, pageErr.message || pageErr);
        }
      }
